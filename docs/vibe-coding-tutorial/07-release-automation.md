...
```
*(Create various task skills following the official guide so documents can be used continuously. Reorganize documents per the official guide.)*
...
```
*(Create a release skill too. When a release is requested, run code review, doc review, test review, security review, cost optimization review, and operational review in parallel.)*
...
```
*(The structure changed a lot, so there might be broken links. Check all document links including README.)*
...
```
*(Is the cold start reduction because Lambda containers load faster than Fargate by default?)*

```
SNS상의 질문에 답변할 수 있게 500자 이내의 서술형으로 정리해줘.
```
*(Summarize in under 500 words so I can answer the SNS question.)*
